...
class Tea(layers.Layer):
    ...
    Keyword Arguments:
        ...
        input_width --  Can be called num_steps, ticks. Number of iterations when encoding inputs. 
                        The more number of iterations, the more spike inputs are close to real inputs.
                        DEFAULT: 1
    ...
    def call(self, inputs):
        if (input.shape [1] != self.input_width):
        ...
        #with tf.compat.v1.get_default_graph().gradient_override_map({"Round":"CustomRound"}):
            # Fixed with tf2.Graph()
            with tf.Graph().as_default().gradient_override_map({"Round":"CustomRound"}):
            # need to check if can define network input_width in build() method
                
                # Constrain the Input:
                if self.round_input:
                    #inputs_slice = tf.keras.backend.round(inputs[:,tick_num])
                    inputs_slice = tf.keras.backend.round(inputs)
                else:
                    #inputs_slice = tf.keras.backend.in_train_phase(inputs[:,tick_num], tf.keras.backend.round(inputs[:,tick_num]))
                    inputs_slice = tf.keras.backend.in_train_phase(inputs, tf.keras.backend.round(inputs))
                ...
                output = tf.zeros(inputs.shape, type(inputs))
                for tick_num in range(self.input_width):
                    
                    # Dot Product the Input with the Weighted Connections
                    output = tf.keras.backend.dot(inputs_slice[:,tick_num], weighted_connections)
                    
                    # Apply activation / Spike(s)
                    output = tf.keras.backend.in_train_phase(self.activation(output), 
                                                             tf.dtypes.cast(tf.math.greater_equal(output, 0.0), 
                                                             tf.float32))
